...
LineWrtLine LineOnSide(const WorldLine& line, const WorldLine& dividing_line)
{
    ...
    if (point_on_side_1 == PointWrtLine::BACK && point_on_side_2 == PointWrtLine::FRONT)
    {
        return LineWrtLine::INTERSECT;
    }
    else if (point_on_side_1 == PointWrtLine::FRONT && point_on_side_2 == PointWrtLine::BACK)
    {
        return LineWrtLine::INTERSECT;
    }
    else if (point_on_side_1 == PointWrtLine::FRONT || point_on_side_2 == PointWrtLine::FRONT)
    {
        return LineWrtLine::FRONT;
    }
    else if (point_on_side_1 == PointWrtLine::BACK || point_on_side_2 == PointWrtLine::BACK)
    {
...
SplitResult SplitLine(const WorldLine& line, const WorldLine& split_line)
{
    ...
    QLineF::IntersectType intersect_type =
        QLineF(line.p0(), line.p1()).intersect(QLineF(split_line.p0(), split_line.p1()), &intersection);
    ...
    {
        return SplitResult{WorldLine{QLine{line.p0(), intersection.toPoint()}, line.normal()},
                           WorldLine{QLine{intersection.toPoint(), line.p1()}, line.normal()}};
    }
    else
    { // P1 in front
        return SplitResult{WorldLine{QLine{intersection.toPoint(), line.p1()}, line.normal()},
